...
// QueryEkycs chains the current query on the "ekycs" edge.
func (aq *AccountQuery) QueryEkycs() *EkycQuery {
	query := &EkycQuery{config: aq.config}
...
func (aq *AccountQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (aq *AccountQuery) FirstIDX(ctx context.Context) uint {
...
func (aq *AccountQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (aq *AccountQuery) OnlyIDX(ctx context.Context) uint {
...
func (aq *AccountQuery) IDs(ctx context.Context) ([]uint, error) {
	var ids []uint
...
func (aq *AccountQuery) IDsX(ctx context.Context) []uint {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// WithEkycs tells the query-builder to eager-load the nodes that are connected to
// the "ekycs" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithEkycs(opts ...func(*EkycQuery)) *AccountQuery {
	query := &EkycQuery{config: aq.config}
...
//		GroupBy(account.FieldAccountName).
//		Aggregate(gen.Count()).
//		Scan(ctx, &v)
...
//
//	client.Account.Query().
//		Select(account.FieldAccountName).
//		Scan(ctx, &v)
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withEkycs; query != nil {
		if err := aq.loadEkycs(ctx, query, nodes,
			func(n *Account) { n.Edges.Ekycs = []*Ekyc{} },
			func(n *Account, e *Ekyc) { n.Edges.Ekycs = append(n.Edges.Ekycs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadEkycs(ctx context.Context, query *EkycQuery, nodes []*Account, init func(*Account), assign func(*Account, *Ekyc)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint]*Account)
	nids := make(map[uint]map[*Account]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.EkycsTable)
		s.Join(joinT).On(s.C(ekyc.FieldID), joinT.C(account.EkycsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(account.EkycsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.EkycsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "ekycs" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
